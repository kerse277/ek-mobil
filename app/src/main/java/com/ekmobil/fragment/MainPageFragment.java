...
public class MainPageFragment extends Fragment {
    ...
    private RecyclerView rcy_main_page;
    ...
    private void loadData(List<ImageEntity> list) {
        if (list != null && list.size() > 0) {
            rcy_main_page.setHasFixedSize(true);
            rcy_main_page.setLayoutManager(new GridLayoutManager(nActivity, 3));
            ImageAdapter adapter = new ImageAdapter(nActivity, list);
            rcy_main_page.setAdapter(adapter);
        } else {
    ...
    private void defineComponents(View view) {
        rcy_main_page = view.findViewById(R.id.main_page_rcy);
